...
export default function AnnotatedImage(props: AnnotatedImageProps): JSX.Element {
  ...
  const convertPixels = useCallback(({ x, y }: Coords) => {
  ...
  const onCompleteDrag = useCallback(({ dragStart, dragEnd }) => {
  ...
  return (
    ...
    >
      ...
      {drawingRect && (
        <rect x={dragStart.x} y={dragStart.y} width={100} height={100} style={{ ...baseRectStyles, ...drawingRectStyles }} />
      )}
      {drawingLine && (
        <line style={{ ...baseLineStyles, ...drawingLineStyles }} x1={dragStart.x} y1={dragStart.y} x2={current.x} y2={current.y} />
      )}
      {rects.map(rect => (
        <rect key={rect.id} style={baseRectStyles} x={rect.bounds[0]} y={rect.bounds[1]} width={rect.bounds[2]} height={rect.bounds[3]} />
      ))}
      {lines.map(line => (
      <line key={line.id} style={baseLineStyles} x1={line.bounds[0]} y1={line.bounds[1]} x2={line.bounds[2]} y2={line.bounds[3]} />
      ))}
    </svg>
  )
